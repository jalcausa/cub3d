...
/*
 * Stores each scene parameter value (textures and colors).
 * Ensures each parameter is only assigned once.
 * Returns an error if the parameter is unknown or repeated.
 */
...
/*
 * Calculates the maximum length among all map lines.
 * Ignores the final newline character if present in each line.
 * Useful for allocating the map matrix.
 */
...
/*
 * Validates that the 6 required parameters are set before the map.
 * Calculates dimensions and stores the map in the scene.
...
/*
 * Parses the entire config file: first parameters, then the map.
 * Calls auxiliary functions for each part.
